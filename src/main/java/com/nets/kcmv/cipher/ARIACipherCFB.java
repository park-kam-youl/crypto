...
public class ARIACipherCFB extends CipherSpi {
    ...
    public ARIACipherCFB() throws InvalidKeyException {
    ...
    protected void engineInit(int opmode, Key key, AlgorithmParameterSpec params, SecureRandom random) throws InvalidKeyException, InvalidAlgorithmParameterException
    {
        ...
        engine.setupEncRoundKeys(); // CFB uses encryption engine for both encryption and decryption
        buffer.reset();
    }
    ...
    private void processBlock(byte[] in, int inOff, byte[] out, int outOff) {
        try {
            ...
            if (forEncryption) {
                for (int j = 0; j < 16; j++) {
                    out[outOff + j] = (byte) (in[inOff + j] ^ keystream[j]);
                }
                System.arraycopy(out, outOff, feedback, 0, 16);
            } else {
                System.arraycopy(in, inOff, feedback, 0, 16);
                for (int j = 0; j < 16; j++) {
                    out[outOff + j] = (byte) (in[inOff + j] ^ keystream[j]);
                }
            }
