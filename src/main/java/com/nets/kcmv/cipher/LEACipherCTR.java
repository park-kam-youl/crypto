...
public class LEACipherCTR extends CipherSpi
{
    ...
    protected void engineInit(int opmode, Key key, AlgorithmParameterSpec params, SecureRandom random) throws InvalidKeyException, InvalidAlgorithmParameterException
    {
        ...
        {
            throw new InvalidAlgorithmParameterException("IV length must be " + engine.getBlockSize() + " bytes for LEA/CTR");
        }
        System.arraycopy(iv, 0, this.counter, 0, engine.getBlockSize());
        this.byteCount = 0;

        engine.init(LeaEngine.Mode.ENCRYPT, key.getEncoded()); // CTR uses encryption engine for both modes
        buffer.reset();
    }
    ...
    protected byte[] engineUpdate(byte[] input, int inputOffset, int inputLen)
    {
        ...
        for (int i = 0; i < data.length; i++)
        {
            if (byteCount == 0)
            {
                engine.processBlock(counter, 0, keystreamBlock, 0);
                incrementCounter();
            }
