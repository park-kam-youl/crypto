...
public class OFBMode implements BlockCipherMode {
    ...
    public OFBMode(BlockCipherEngine engine) {
        this.engine = engine;
    ...
    public byte[] update(byte[] input, int inputOffset, int inputLen) {
        ...
        for (int i = 0; i < numBlocks; i++) {
            try {
                ...
                throw new IllegalStateException("Error during OFB update operation", e);
            }
    ...
    public byte[] doFinal(byte[] input, int inputOffset, int inputLen) throws IllegalBlockSizeException, BadPaddingException {
        ...
        for (int i = 0; i < data.length; i += engine.getBlockSize()) {
            try {
                ...
                throw new IllegalStateException("Error during OFB doFinal operation", e);
            }
    ...
    public int getOutputSize(int inputLen) {
        ...
        return total; // OFB mode output size is always the same as input size, padding is handled separately
    }
