...
public class HIGHTCipherGCM extends CipherSpi {
    ...
    protected void engineInit(int opmode, Key key, AlgorithmParameterSpec params, SecureRandom random) throws InvalidKeyException, InvalidAlgorithmParameterException {
        ...
        if (this.macSize < 4 || this.macSize > 8) { // HIGHT block size is 8 bytes
    ...
    private void gmult(byte[] x, byte[] y, byte[] z) {
        ...
        for (int i = 0; i < engine.getBlockSize(); i++) {
            for (int j = 0; j < 8; j++) { // This loop is incorrect, should be 8
                if ((x[i] & (1 << (7 - j))) != 0) {
                    xor(z, z, v);
                }
