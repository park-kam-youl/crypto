...
public class SEEDCipherCBC extends CipherSpi {
    ...
    public SEEDCipherCBC() {
        this.feedback = new byte[16];
    ...
    protected byte[] engineDoFinal(byte[] input, int inputOffset, int inputLen) throws IllegalBlockSizeException, BadPaddingException {
        ...
        if (forEncryption) {
            ...
            for (int i = 0; i < padded.length; i += engineGetBlockSize()) {
                ...
                engine.encrypt(feedback, 0, feedback, 0);
                System.arraycopy(feedback, 0, finalOutput, i, engineGetBlockSize());
            }
        } else {
            ...
            for (int i = 0; i < data.length; i += engineGetBlockSize()) {
                byte[] temp = new byte[engineGetBlockSize()];
                System.arraycopy(data, i, temp, 0, engineGetBlockSize());
    ...
    private void processBlock(byte[] in, int inOff, byte[] out, int outOff) {
        if (forEncryption) {
            ...
            engine.encrypt(feedback, 0, feedback, 0);
            System.arraycopy(feedback, 0, out, outOff, 16);
        } else {
            byte[] temp = new byte[16];
            System.arraycopy(in, inOff, temp, 0, 16);
            engine.decrypt(in, inOff, out, outOff);
            for (int i = 0; i < 16; i++) {
                out[outOff + i] ^= feedback[i];
            }
            feedback = temp;
        }
    }
