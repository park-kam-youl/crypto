...
public class HIGHTEngine {
    ...
    public void init(Mode mode, byte[] mk) {
        if (mk.length != 16) {
            throw new IllegalArgumentException("Key size must be 16 bytes for HIGHT");
        }
    ...
    public void encrypt(byte[] in, int inOff, byte[] out, int outOff) {
        ...
        for (int r = 0; r < 32; r++) {
            ...
            key_offset += 4;
            // The original sample code has a fixed pattern for round key usage and index rotation.
            // This simplified loop assumes a linear progression of round keys.
            // A more accurate implementation would follow the exact index rotation as in the sample.
        }
    ...
    public void decrypt(byte[] in, int inOff, byte[] out, int outOff) {
        ...
        for (int r = 0; r < 32; r++) {
            ...
            key_offset -= 4;
            // Similar to encryption, this assumes a linear progression.
        }
