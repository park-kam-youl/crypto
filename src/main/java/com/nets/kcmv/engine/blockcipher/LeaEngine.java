...
public class LeaEngine
{
    ...
    // pack and unpack methods from Ops.java, adapted for LeaEngine
    private static final void pack(byte[] in, int inOff, int[] out, int outOff, int inlen)
    {
        if (in == null || out == null)
        {
    ...
    private static final void unpack(int[] in, int inOff, byte[] out, int outOff, int inlen)
    {
        if (in == null || out == null)
        {
            throw new NullPointerException();
